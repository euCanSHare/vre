...
function getEGADatasetsFromDSid($var){
	$response="{}";
        $curl = curl_init();             
        curl_setopt($curl, CURLOPT_URL, 'https://test.ega-archive.org/metadata/v2/datasets/'.$var.'?idType=EGA_STABLE_ID');
//        curl_setopt($curl, CURLOPT_URL, 'https://test.ega-archive.org/metadata/v2/datasets/EGAD50000000045?idType=EGA_STABLE_ID');
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
        $response = curl_exec($curl);
        $response = json_encode($response);
        curl_close($curl);
...
function getEGAFilesFromDSid($var){
	$response="{}";
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, 'https://test.ega-archive.org/metadata/v2/files?queryBy=dataset&queryId='.$var.'');
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
        $response = curl_exec($curl);
        $response = json_encode($response);
        curl_close($curl);
